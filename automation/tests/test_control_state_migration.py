...
class ControlStateMigrationTests(unittest.TestCase):
    def setUp(self):
        ...
        with sqlite3.connect(self.source) as connection:
            ...
            connection.executemany(
                ...
                ((version, "2026-07-15T14:00:00Z") for version in range(1, 10)),
            )
            connection.execute(
    ...
    def test_read_only_audit_and_isolated_rehearsal_preserve_source(self):
        ...
        self.assertEqual(result.source_schema_version, 9)
        ...
        self.assertEqual(sha256(self.source), before)
        self.assertEqual(audit_control_state(self.source).schema_version, 9)
        self.assertEqual(
