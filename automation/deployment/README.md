...
The local activation audit consumes a short-lived private proof generated by
querying the exact schedule and job. It records only paused state, zero active
executions, and an observation timestamp; tracked code rejects proofs older
than 15 minutes or carrying resource identifiers. A retained proof is evidence,
not permission to mutate either cloud or local state.
