...
class EMNLPScraper(BaseScraper):
   """EMNLP scraper - copy this to create new conference scrapers."""


   def __init__(self):
       super().__init__('emnlp')
   def labeled_json_exists(self) -> bool:
       return os.path.exists("data/emnlp/labeled.json")

   def load_labeled_tracks(self) -> dict:
       with open("data/emnlp/labeled.json") as f:
           return json.load(f)
   ...
   def get_paper_urls(self, year: int) -> List[str]:
       ...
       os.makedirs("data/emnlp", exist_ok=True)
       with open(f"data/emnlp/unlabeled_{year}.json", "w") as f:
           json.dump(unlabeled_json, f, indent=2)
       logger.warning(
           f"Unlabeled track list for {year} written to data/emnlp/unlabeled_{year}.json. "
           "Please label this file using GPT and save as data/emnlp/labeled.json, then rerun."
       )
       exit(0)
