...
Track filtering:
  ...
  classification, and the result is cached in data/cache/naacl_tracks.json.
...
_LABELED_PATH = "data/cache/naacl_tracks.json"
...
class NAACLScraper(BaseScraper):
    """NAACL conference scraper using ACL Anthology."""
    ...
    def get_paper_urls(self, year: int) -> List[str]:
        """Return paper URLs for a given NAACL year.

        On first run for a given year, calls the Gemini API to identify
        main-conference tracks and caches the result in data/cache/naacl_tracks.json.
        """
        logger.info(f"Getting NAACL {year} paper URLs...")

        all_tracks = self.get_track_names(year)
        if not all_tracks:
            logger.error(f"No tracks found for NAACL {year}")
            return []
        ...
        logger.info(f"Found {len(paper_urls)} papers for NAACL {year}")
        return paper_urls

    def parse_paper(self, url: str) -> Optional[Dict]:
        """Parse a single NAACL paper from its ACL Anthology page."""
        try:
            response = self.session.get(url)
            if not response:
                return None
    ...
    def get_track_names(self, year: int) -> list:
        """Return all track names listed on the NAACL event page for a given year."""
        url = f"{self.base_url}/events/naacl-{year}/"
        response = self.session.get(url)
        if not response:
    ...
    def _get_relevant_tracks(self, year: int, all_track_names: list) -> list:
        ...
        Loads from data/cache/naacl_tracks.json if the year is already cached.
        ...
        if year_str not in labeled:
            logger.info(f"No labeled data for NAACL {year}. Attempting auto-labeling...")

            year_data = self._auto_label(year, all_track_names)

            if not year_data:
                logger.warning(f"Auto-labeling could not be completed for NAACL {year}.")
                year_data = {
                    "tracks": [{"name": name, "is_full_regular": False} for name in all_track_names]
                }
                labeled[year_str] = year_data
                self._save_labeled(labeled)
                logger.error(f"Auto-labeling failed for NAACL {year}. Please manually edit {_LABELED_PATH} and set 'is_full_regular': true for main tracks, then rerun.")
                return []
    ...
    def _auto_label(self, year: int, track_names: list) -> Optional[dict]:
        ...
        try:
            ...
            if "tracks" not in result or not isinstance(result["tracks"], list):
                logger.error(f"Unexpected JSON structure from Gemini for NAACL {year}")
                return None

            main = [t["name"] for t in result["tracks"] if t.get("is_full_regular")]
            logger.info(f"Auto-labeled NAACL {year} via {self.model_name}: main tracks = {main}")
            return result
