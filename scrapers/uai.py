...
class UAIScraper(BaseScraper):
    """UAI conference scraper using MLR Press volumes."""
    ...
    def _get_volume_for_year(self, year: int) -> Optional[str]:
        """Get UAI volume identifier (e.g., 'v201') for the given year."""
        ...
        try:
            ...
            # Search for UAI proceedings (same pattern as ICML)
            uai_pattern = re.compile(
                rf'\b(?:Proceedings\s+of.*?UAI\s+{year})\b',
                re.IGNORECASE
            )

            for li in soup.find_all('li'):
                li_text = li.get_text().strip()
                if uai_pattern.search(li_text):
                    ...
                    if link:
                        ...
                        if match:
                            ...
                            logger.info(f"Found UAI {year} volume: {volume_id}")
                            return volume_id
            ...
            logger.error(f"Error while fetching UAI volume for year {year}: {e}")
            return None
    
    def get_paper_urls(self, year: int) -> List[str]:
        """Get paper URLs for UAI year."""
        logger.info(f"Getting UAI {year} paper URLs...")
    ...
    def parse_paper(self, abs_url: str) -> Optional[Dict]:
        """Parse a single UAI paper from its abstract URL."""
        try:
