...
def scan(metadata_root: Path, data_root: Path):
    ...
    for path in sorted(metadata_root.glob("*/*.json")):
        ...
        row["provisional_papers"] = sum(
            paper.get("publication_status") == "provisional" for paper in papers)
...
def render_readme_coverage(stats):
    ...
    for conf in confs:
        archival = [
            year for year, row in stats[conf].items()
            if not row.get("provisional_papers")
        ]
        provisional = [
            year for year, row in stats[conf].items()
            if row.get("provisional_papers")
        ]
        parts = []
        if archival:
            parts.append(format_years(archival))
        if provisional:
            parts.append(f"provisional: {format_years(provisional)}")
        lines.append(
            f"- **{NAMES.get(conf, conf.upper())}** ({'; '.join(parts)})")
    return "\n".join(lines)
...
def render(stats):
    ...
    for conf in confs:
        ...
        grand.update(totals)
        provisional_years = sum(
            bool(row.get("provisional_papers")) for row in rows)
        grand["provisional_years"] += provisional_years
        out.append(
            f"| {NAMES.get(conf, conf.upper())} | {format_years(stats[conf])} | "
            f"{provisional_years:,} | "
            f"{totals['papers']:,} | {totals['pdfs']:,} | {totals['missing_abstract']:,} |")
    out.append(
        f"| **Total** | | **{grand['provisional_years']:,}** | "
        f"**{grand['papers']:,}** | **{grand['pdfs']:,}** | "
    ...
    for conf in confs:
        ...
        for year, row in sorted(stats[conf].items()):
            status = "provisional" if row.get("provisional_papers") else "archival"
            out.append(f"| {year} | {status} | " + " | ".join(
